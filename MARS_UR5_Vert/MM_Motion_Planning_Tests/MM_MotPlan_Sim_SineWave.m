...
MotPlan = struct([]);
%Use the sine wave path trajectory function
MotPlan=SineWavePath(T0,tf,ts,tb,d,h,Nwaves);
...
% Create the final pose transformation matrix
...
if k==N
    ...
    xi_pos_error = xi_pos_error(:,1:k);
    xi_orient_error = xi_orient_error(:,1:k);
    q = q(:,1:k);
    dq = dq(:,1:k);
    eta = eta(:,1:k);
    dist_elbow = dist_elbow(1:k);
    dist_wrist = dist_wrist(1:k);
    wrist_pos = wrist_pos(:,1:k);
