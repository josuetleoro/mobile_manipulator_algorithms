classdef DualQuat
   ...
   methods(Static)
       function obj=RigidTransf(angle,d,m)
           aux=angle*0.5;
           obj=DualQuat(Quat(cos(aux),sin(aux)*d),Quat(0,sin(aux)*m));
       end
       
       function obj=point2DualQuat(p)
          obj=DualQuat(Quat(1,[0,0,0]),Quat(0,p)); 
       end
       
       function obj=vector2DualQuat(v)
          obj=DualQuat(Quat(0,[0,0,0]),Quat(0,v)); 
       end
